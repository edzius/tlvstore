...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "F:S:hgsc", tlvstore_options, &index)) != -1) {
		switch (opt) {
		case 'F':
	...
	ldebug("Opening store file: %s (size: %d)", store_file, store_size);
	...
	ldebug("TLV store initialized at %p, size %zu", tlvd->base, tlvd->size);
